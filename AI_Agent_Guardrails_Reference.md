# AI Agent Guardrails — Practitioner's Reference

*Techniques, frameworks, products & research — organized by control layer, with concrete tools and academic references for each. Converted from the original research PDF for guardrail-development use.*

> **Scope note:** Per request, the *skill / MCP supply-chain attack* sub-topic (malicious `SKILL.md` files / malicious MCP server definitions altering agent behavior on load) has been removed from this version, since that is being handled separately and isn't part of the guardrail work being built from this doc. Everything else from the source PDF — input/output/dialog/tool/retrieval/multi-agent rails, information-flow control, classifiers, constitutional approaches, framework comparisons, and the evaluation/benchmark section — is kept, since the benchmark section is exactly the "testing methods on security methods" material.

---

## Contents

1. [Why agent guardrails — the threat landscape](#1-why-agent-guardrails--the-threat-landscape)
2. [A taxonomy: where guardrails sit in an agent](#2-a-taxonomy-where-guardrails-sit-in-an-agent)
3. [Input guardrails — prompt injection, jailbreaks, PII](#3-input-guardrails--prompt-injection-jailbreaks-pii)
4. [Output guardrails — content, hallucination, schema](#4-output-guardrails--content-hallucination-schema)
5. [Dialog & topic guardrails — staying on rails](#5-dialog--topic-guardrails--staying-on-rails)
6. [Tool & action guardrails — least privilege, sandboxing, HITL](#6-tool--action-guardrails--least-privilege-sandboxing-hitl)
7. [Retrieval & memory guardrails — RAG poisoning, MINJA](#7-retrieval--memory-guardrails--rag-poisoning-minja)
8. [Multi-agent communication guardrails](#8-multi-agent-communication-guardrails)
9. [Information flow control — CaMeL, FIDES, dual-LLM](#9-information-flow-control--camel-fides-dual-llm)
10. [Specialized safety classifier models](#10-specialized-safety-classifier-models)
11. [Constitutional & policy-driven approaches](#11-constitutional--policy-driven-approaches)
12. [Production frameworks compared](#12-production-frameworks-compared)
13. [Evaluation & benchmarks — testing methods](#13-evaluation--benchmarks--testing-methods)
14. [Reference defense-in-depth architecture](#14-reference-defense-in-depth-architecture)
...
### 1.1 Reference attack surfaces

- **Prompt injection (direct & indirect):** attacker text smuggled into a user message, retrieved document, tool output, web page, email, or calendar invite causes the agent to follow attacker instructions instead of the user's.
...
- **Tool misuse & confused deputy:** the agent has legitimate credentials but is tricked into invoking them against attacker-chosen targets (sending email to the attacker, transferring funds, deleting data).
...
- **OWASP Top 10 for LLM Applications (2025)** — prompt injection, insecure output handling, sensitive info disclosure, excessive agency, etc.
...
| 5. Retrieval / memory rails | What gets pulled into the prompt from RAG and long-term memory | Source provenance, signed/labeled chunks, write-time moderation, retrieval-time validators |
...
Two cross-cutting design principles recur in every recent paper and vendor guide:

- **Defense-in-depth, not single point of trust.** Anthropic's Constitutional Classifiers describe a "Swiss-cheese" model — each layer has holes but they don't line up. Multi-Agent Defense Pipelines (arXiv:2509.14285) report Attack Success Rate (ASR) going from 30% to 0% when several specialized detector agents are stacked. No single classifier is robust enough on its own.
- **Treat all external content as untrusted by default.** This is the core insight behind boundary-awareness prompting, spotlighting, StruQ, dual-LLM, and CaMeL. The agent's reasoning context is a security boundary; anything that crosses it from outside must be labeled, sanitized, or sandboxed.
...
- **Spotlighting** (Hines et al., Microsoft, arXiv:2403.14720). Make provenance salient by delimiting, datamarking (inserting an unguessable token between every word of untrusted text), or encoding untrusted spans (base64). Drops attack success from >50% to <2% on GPT-family in their experiments.
...
- **AgentSentry** (arXiv:2602.22724). Models multi-turn indirect injection as a "temporal causal takeover" and localizes which earlier tool output drove the current bad action, enabling context purification instead of a full session kill.
...
| ShieldGemma 1 / 2 (Google) | Gemma-2 (2B, 9B, 27B); Gemma-3 for v2 | +10.8% AU-PRC vs. Llama Guard on public benchmarks per Google. v2 also moves to multimodal (image) classification |
| Aegis-Guard (NVIDIA) | LoRA on Llama-Guard | Defensive vs. Permissive variants for tuning the precision/recall tradeoff |
| WildGuard (AI2) | Llama-3 8B | Three goals at once: malicious prompt, response harm, refusal detection; reports a sizeable F1 gain over prior open moderation tools |
| Qwen3Guard (Alibaba) | 0.6B – 8B | 119 languages, three-tier severity (safe / controversial / unsafe) |
| Constitutional Classifiers (Anthropic) | Trained from a written "constitution" | Cut universal-jailbreak success from 86% to 4.4%; next-gen "exchange" classifier (see §11.2 for detail) |
...
- Strip / sanitize agent-emitted markdown links and images before they're rendered in a UI — a common exfiltration vector (image `src` with stolen data in the URL).
...
If "what the agent says" is the output rail, "what the agent does" is the action rail — and it carries the highest blast radius. OWASP's Top 10 for Agentic Applications lists tool misuse and excessive agency as top risks. Three principles dominate the literature: least privilege, deny-by-default, and human confirmation for irreversible actions.
...
- **NVIDIA's 2026 guidance on sandboxing agentic workflows** stresses: ephemeral creds, blast-radius limits per task, no long-running sandboxes that accumulate dependencies, and aggressive secret rotation.
...
- **Memory-write requires an explicit reasoning step by the agent**, which can be inspected by a critic before commit.
...
- **Signed / authenticated inter-agent messages.** Mutual TLS + per-agent identity.
...
Detection-based defenses can always be probabilistically bypassed by a new attack. IFC-style defenses convert a probabilistic problem into a structural one: even a perfectly successful jailbreak cannot reach a forbidden sink. The cost is engineering complexity and reduced utility (CaMeL's default strict policy rejects any tool call whose arguments contain untrusted data, which hurts task completion). "Operationalizing CaMeL" (arXiv:2505.22852) discusses practical extensions: plan-template caching, deterministic micro-parsers replacing the Q-LLM where possible, and side-channel mitigations.
...
| ShieldGemma 2 (Google) | Multimodal | Image classification for sexually explicit, dangerous, violent/gory content |
| Constitutional Classifiers / CC++ (Anthropic) | Cascaded LLM classifier + activation probes | Trained from a written "constitution." CC++ uses activation probes for cheap first-pass filtering before the full classifier |
...
| Galileo Luna-2 SLMs | Small specialized models | Hallucination detection at 152 ms / 88% accuracy; large cost reduction vs. LLM-as-judge approaches |
| GuardAgent (arXiv:2406.09187) | Agent-based | A guard agent that uses knowledge-enabled reasoning over a safety policy to decide whether actions are safe |
...
- Next-generation (2026): a single "exchange" classifier sees input+output context together — catches attacks like "food flavorings" (a coded reference to reagents) that look benign in isolation. 87% reduction in over-refusals. Activation probes added as a cheap first-stage filter.
...
### 11.4 Reported cost impact

Industry analyses (Nexastack 2026, Atlan 2026) attribute most reported compliance-cost savings (40–70% in some accounts) to (a) reusing one policy across many services, (b) automatic audit-log generation, and (c) the ability to roll back policy changes the same way you'd roll back code.

---

## 12. Production frameworks compared

Practical advice consolidated from vendor comparisons (Galileo's Top 8 in 2026, FutureAGI's 6-platform comparison, Maxim AI's Bifrost guide, ToolHalla's 5-layer stack guide, WorkOS overview). Current as of mid-2026.

| Framework / Product | License | Niche / strength |
|---|---|---|
| NVIDIA NeMo Guardrails | Apache-2.0 | Dialog flows with Colang, 5 rail types, event-driven runtime; best when you need fine-grained dialog-flow control |
| Guardrails AI (OSS + Pro) | Apache-2.0 / commercial | Validator hub, JSON-schema + reask loop, Pydantic integration. Best for structured-output enforcement |
| OpenAI Guardrails Python (2025) | Open source | Drop-in client wrapper with input/output validation; configured via a Guardrails Wizard |
...
| Lakera Guard | Commercial API | Prompt injection / jailbreak / PII at <50 ms, 100+ languages, daily retraining from Gandalf adversarial data |
| LLM Guard (Protect AI) | MIT | Open self-hosted scanner suite (input + output). Free baseline for most use cases |
| Vigil / Rebuff | OSS | YARA + transformer + vector-DB + canary-word; lower performance than Lakera per public comparisons |
...
| FutureAGI Protect + Agent Command Center | Commercial | Two-layer architecture: four Gemma-3n LoRA adapters + an OpenAI-compatible gateway |
| Maxim AI Bifrost | Commercial gateway | Multi-provider guardrail integration (Bedrock, Azure, Patronus, GraySwan) with centralized evaluation/observability |
| NeuralTrust | Commercial | Constitutional-classifier-style protection |
| WorkOS / Cequence / Strata / Gravitee | Commercial | Identity/authorization layer for agents — OAuth 2.1 + Dynamic Client Registration, downscoped tokens |
| Cerbos / OPA + Rego | Apache-2.0 | Policy-as-code decision points evaluated at every tool call; deny-by-default |

---

## 13. Evaluation & benchmarks — testing methods

Guardrails fail in two directions — too loose (low recall on adversarial inputs, harm passes) and too strict (low precision on benign inputs, legitimate requests blocked). A single mixed-set F1 score hides which failure mode your stack has. **Evaluation must be split** by failure mode, not collapsed into one number.

### 13.1 Agent security benchmarks

| Benchmark | Focus | Notes |
|---|---|---|
| AgentDojo (NeurIPS 2024) | Indirect prompt injection | 97 user tasks, 629 security cases across Workspace/Slack/Travel/Banking environments |
| InjecAgent (Zhan et al., 2024) | Direct harm + data theft via IPI | Isolated-step evaluation; lighter than AgentDojo |
| Agent Security Bench / ASB (Zhang et al., 2024-25) | Attacks + defenses | 50 tasks across 10 domains (finance, law, education); standardized adversarial setup |
...
| AgentDyn (2026) | Dynamic real-world settings | Open-ended benchmark; addresses the gap between sterile benchmark wins and real-world robustness |
...
- **False-positive / over-refusal rate** on benign inputs — the Constitutional Classifiers paper treats this as primary, not secondary.
- **Per-layer latency budget** — input rails <30 ms, dialog <200 ms, output <50 ms (ToolHalla 2026 reference).
- **Hallucination / groundedness rate**, ideally measured against a reference store.
...
> This section is the core "testing methods on security methods" material — use it to decide what you measure once the guardrails below are built, and run benchmarks like AgentDojo / WASP / AgentHarm against your own stack rather than relying on vendor-reported numbers alone.

---

## 14. Reference defense-in-depth architecture

A synthesis of the techniques above. This is the general topology mature production agent stacks use in 2026 — exact component names vary, but the layer order and intent are consistent.
...
└──────────────────────────────────────────────────────────┘
        ...
        ▼
┌──────────────────────────────────────────────────────────┐
│ L2 — DIALOG / TOPIC RAILS                (target 50-200ms)│
...
│  • Task-Shield checks each action vs. the user's task       │
└──────────────────────────────────────────────────────────┘
        │
        ▼
┌──────────────────────────────────────────────────────────┐
...
└──────────────────────────────────────────────────────────┘
        ...
        ▼
┌──────────────────────────────────────────────────────────┐
│ OBSERVABILITY (cross-cutting, every layer reports here)       │
...
- Turn on a managed content classifier for outputs (Bedrock / Azure Content Safety / self-hosted Llama Guard).
...
- **Over-blocking.** Each new rail you add increases the chance of refusing legitimate users. Track false-positive rate (FPR) per rail, not just ASR.
- **Defense flattening over long contexts.** Many classifiers degrade as the context grows past ~4k tokens — exactly where indirect prompt injection lives.
- **Single-classifier dependence.** Adaptive attacks routinely break any one classifier. Constitutional Classifiers, information-flow control, and HITL exist because no single check is sufficient.
...
Selected papers, standards, and product documentation. Where multiple versions exist (e.g., Llama Guard 1–4), the most recent and the original are both listed.
